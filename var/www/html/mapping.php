...
if (isset($_GET["command"]) && $_GET["command"] === 'delete') {
    ...
    if (strpos($deletefile, '/etc/openjvs/games/') === 0 && preg_match('/^[a-zA-Z0-9_\-]+$/', $filename)) {
...
$mappingfiles = is_dir('/etc/openjvs/games') ? scandir('/etc/openjvs/games') : [];
    ...
    echo '<html lang="en"><head><meta charset="utf-8"><title>WiPi Netbooter - JVS Mappings</title>';
    ...
    echo '<h1>' . arcade_icon('gamepad') . ' OpenJVS Mapping Files</h1>';
    echo '<p class="page-intro">Manage game mapping files for OpenJVS.</p>';
    ...
    echo '<div class="card">';
    echo '<div class="card-header"><h2 class="card-title">Mapping Files</h2></div>';
    ...
    for ($i = 2; $i < count($mappingfiles); $i++) {
        $name = $mappingfiles[$i];
        $path = '/etc/openjvs/games/' . $name;
        echo '<tr><td>' . htmlspecialchars($name, ENT_QUOTES, 'UTF-8') . '</td>';
        echo '<td><a href="editor.php?mode=jvs&mappingfile=' . urlencode($path) . '" class="btn btn-secondary btn-sm">Edit</a> ';
        echo '<a href="mapping.php?command=delete&filetodelete=' . urlencode($path) . '" class="btn btn-danger btn-sm" onclick="return confirm(\'Delete ' . htmlspecialchars($name, ENT_QUOTES, 'UTF-8') . '?\')">Delete</a></td></tr>';
    }
    echo '</tbody></table>';
    echo '</div>';
