...
echo '<html lang="en"><head><meta charset="utf-8"><title>WiPi Netbooter - OpenFFB Update</title>';
...
echo '<h1 class="text-3xl" style="margin-bottom: 24px;">' . arcade_icon('refresh') . ' OpenFFB Updater</h1>';

if ($confirm !== 'yes') {
    echo '<div class="card" style="max-width: 520px;">';
    echo '<div class="card-header"><h2 class="card-title">Update OpenFFB</h2></div>';
    echo '<div class="card-body">';
    echo '<p style="margin-bottom: 16px;">This will update OpenFFB to the latest version from the project repository.</p>';
    echo '<p style="margin-bottom: 24px;"><strong>Are you sure?</strong></p>';
    echo '<div style="display: flex; gap: 12px; flex-wrap: wrap;">';
    echo '<form action="updateopenffb.php?confirm=yes" method="post"><button type="submit" class="btn btn-primary">' . arcade_icon('lightning') . ' Confirm</button></form>';
    echo '<form action="openffb.php"><button type="submit" class="btn btn-secondary">Cancel</button></form>';
    echo '</div>';
    echo '</div></div>';
} else {
    ...
    pclose($handle);
    echo '</pre>';
    echo '<p style="margin-top: 20px;"><a href="openffb.php" class="btn btn-primary">' . arcade_icon('setup') . ' Return to OpenFFB</a></p>';
    echo '</div></div>';
}
